...
**Reference**: [[../../../docs/rule/[STANDARD]_MJ_Agent_AI_Engineering_Execution_HITL_Prompt|HITL_Prompt v1.0]] §3.1 必停规则（`实现中 scope 明显扩大`）+ §4.4 Repo Scan 反向扫描原理（drift detection 的对偶）.
...
- [[../../../docs/rule/[STANDARD]_MJ_Agent_AI_Engineering_Execution_HITL_Prompt|HITL_Prompt v1.0]] §3.1（必停规则之"实现中 scope 明显扩大"）+ §4.4（Repo Scan 反向扫描，drift 的对偶）
