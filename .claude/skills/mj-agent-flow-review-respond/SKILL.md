...
**Reference**: [[../../../docs/rule/[STANDARD]_MJ_Agent_AI_Engineering_Execution_HITL_Prompt|HITL_Prompt v1.1]] §4.13（Stage 15 Rules + Output 6 字段）+ Meta v2.2 §4.7（沿用 v2.0 §4；实操 prompt 见 HITL_Prompt §4.8 + §4.9） 双段约束（修复后须出双段证据）.
...
- [[../../../docs/rule/[STANDARD]_MJ_Agent_Documentation_Meta_Framework|Meta v2.2]] §4.7（修复后须出双段证据；沿用 v2.0 §4 全部规则；实操 prompt 见 HITL_Prompt §4.8 + §4.9）
