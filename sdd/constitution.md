...
> 一旦 promote 为 `state: active`，落入 §"What Claude Must Not Edit Without Approval".
...
|---|---|---|---|---|
| in-source（runtime） | `src/mj_agent/skills/<name>/SKILL.md` | 13-field Agent_Side | `load_skill()` strip frontmatter | Track B（Agent_Side v1.x；Phase M5 后并入 `sdd/adapters/runtime-skill.md`） |
| in-tree（workflow） | `.claude/skills/mj-agent-*/SKILL.md` | 2-field ADR-013 native | Claude Code main process | Track C（Meta v2.x §3.10；Phase M5 后并入 `sdd/adapters/claude-code-skill.md`） |
