...
| G6 | （内置 §4 hard stops）| 4 项专属必停拦截 | manual-canonical(AGENTS + native hard-block hook/rules + Owner-approved manual application route; no automatic trust or hook activation) |
...
| V4 Native-Skill | `scripts/sdd/check_native_skills.py` | Native schema/index and 8 infra freezes | blocking@ci; strict exit1 is an explicit Owner-approved cutover change |
...
| V8 native development entries | `scripts/sdd/check_native_governance.py --surface entries` | AGENTS ×5 + canonical10 + PR template | blocking@ci; new epoch starts at actual first applied CI, no inherited streak |
| V9 native skill resources and frozen contracts | `scripts/sdd/check_native_skills.py --surface resources` | repo resources and peer invocation closure; freezes in V4 | blocking@ci; no source/lock reconcile after ownership transfer |
| V10 native active consumers | `scripts/sdd/check_native_governance.py --surface consumers` | explicit executable consumers reject retired client reads/calls; documentation is separately reviewed | blocking@ci; no generated byte-diff after ownership transfer |
| V11 native project MCP | `scripts/sdd/check_codex_native.py --surface mcp` | exact8, by-name env, config posture, missing/invalid fail | blocking@ci; excludes enforcement warning surface |
| V12 Cross-Carrier | retired with client source/registry/renderer/lock topology | necessary schema/ref/MCP protections carried by V4/V8/V9/V11; P2 semantic review remains migration evidence | withdrawn only on atomic Owner-approved cutover; no skip-as-pass, no permanent P2 review-script dependency |
| V13 native enforcement structure | `scripts/sdd/check_codex_native.py --surface enforcement` | hooks/rules shape and decisions | warning@ci; missing/invalid is FAIL, not SKIP. Native unit-test additions have separately reviewed blocking effect; host NOT_TESTED |
...
以下四项是 canonical10 的运行时子集。Owner 拍板与实际执行路线分别判断；
原生 hook 对受保护编辑硬阻断，AGENTS 自律始终有效。CI 静态检查不替代宿主或实际审批。
批准后按已审阅路线应用；不得通过改模式、换工具或编码绕过。
