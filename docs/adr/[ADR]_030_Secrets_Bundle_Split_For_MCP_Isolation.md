...
- [[[ADR]_008_Co_Deployment_With_Upstream_Warehouse|ADR-008]] — 独立 compose project + 独立 secrets pipeline 的早期决策；本 ADR 把 secrets pipeline 隔离推进到文件粒度
- [[[ADR]_028_MCP_Server_Inventory_And_Governance|ADR-028]] — MCP server inventory + STANDARD MCP_Server_Governance；本 ADR 是 ADR-028 §D.3 "Independent secrets pipeline" 的落地细化
...
- [[../archive/adr/[DEPRECATED]_[ADR]_025_Multi_Environment_And_LLM_Provider_Abstraction|ADR-025（archive）]] — PR-3 把 MCP secrets 加入 single secrets.enc 的初始决策；本 ADR 修订其注入路径声明
