...
| [[decisions/ADR-031_Spec_Anchored_Refactor\|ADR-031 Spec-Anchored Refactor]] | SYS | proposed (state: draft) | Maximum Spec-Anchored Refactor（Phase M0-M6）：把 tri-track STANDARD + 20 active ADR + ~100 篇文档治理语料重构为 SDD Kernel + Capability Package + Business Policy 三支柱架构，落地机器可读 contract + capability lifecycle + CI gates（M0-M6 已落地、#245 闭幕；frontmatter decision/state 待后续 reconcile） |
| [[decisions/ADR-032_Claude_Skill_Schema_Monitoring\|ADR-032 Claude Skill Schema Monitoring]] | WORKFLOW | proposed (state: draft) | 为 `.claude/skills/` ADR-013 native 2-field schema 合规建立 3 层监控：validator gate（V4；M3 warning / M4 blocking）+ PR 模板 A12 提示 + 季度 A6 audit；把原 M3-FU 范围从「修既存偏差」（premise 经 reverify 证伪）重构为「防未来偏差」（frontmatter decision/state 待后续 reconcile） |
