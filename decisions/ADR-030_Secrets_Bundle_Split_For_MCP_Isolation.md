...
- `capabilities/infrastructure/mcp-server-governance/contracts/mcp-server.contract.yml` §credential_rules（原 STANDARD MCP Server Governance v1.0 §3，M6 X5 archived → `archive/rule/[DEPRECATED]_[STANDARD]_MJ_Agent_MCP_Server_Governance_v1_0.md`）— credential mode "wrapped script (env var via cmd)" 行为不变；本 ADR 改的是注入这些 env 的**机制**（从 .env mirror 改为 .enc direct）
