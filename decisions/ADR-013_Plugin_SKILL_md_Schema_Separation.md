...
[[decisions/ADR-012_Two_Track_Documentation_Governance|ADR-012]] 引入双轨治理 + 双 plugin 骨架（`mj-agent-agent-doc` / `mj-agent-code-doc`），并在 [[../../plans/[PLAN]_F_Documentation_Track_Split_And_Plugin_Skeleton|PLAN F]] §V-skel-4 / §V-skel-5 + [[archive/rule/[DEPRECATED]_[STANDARD]_MJ_Agent_Agent_Side_Documentation_Framework_v1.0|Agent_Side v1.0]] §2 中默认假设 marketplace plugin SKILL.md 与 mj-agent 仓内 `src/mj_agent/skills/**/SKILL.md`（in-source SKILL.md）共用同一套 schema（13 字段 frontmatter + 五段式 body）。
...
- 直接前置：[[decisions/ADR-012_Two_Track_Documentation_Governance|ADR-012]]（双轨治理 + 双 plugin 骨架决策）
- 同期联动修订：
  - [[archive/rule/[DEPRECATED]_[STANDARD]_MJ_Agent_Agent_Side_Documentation_Framework_v1.0|Agent_Side v1.0]] §2 + §9（scope note，同 PR 落地）
