...
description: "Stage 17 post-merge cleanup: close the linked issue, update the changelog, open follow-up issues, flip the plan state after Owner approval and trigger branch deletion plus mirror sync; use right after a PR is merged, for post-merge cleanup, PR 合并后收尾."
---
...
5. **EVAL backlog ticket 自动开单**（mj-agent 专属，execution-loop §7.3 Rule 11）
6. schedule 决策
7. branch 清理
8. develop 同步
...
**Reference**: [[../../../sdd/workflows/execution-loop|execution-loop]] §7（Post-merge sedimentation policy；§7.3 Rule 11 EVAL backlog 是 mj-agent 专属）+ [[../../../sdd/lifecycle|lifecycle]] §2（Working-doc lifecycle）+ Git Branch Strategy + PR Description Convention.
...
<!-- codex-route:edge-flow-post-merge-git-branch -->
> Codex route: invoke `$mj-agent-git-branch` (native carrier; call, conditional)

<!-- codex-route:edge-flow-post-merge-git-delete -->
> Codex route: invoke `$mj-agent-git-delete` (native carrier; call, always)

<!-- codex-route:edge-flow-post-merge-git-sync -->
> Codex route: invoke `$mj-agent-git-sync` (native carrier; call, always)
...
```markdown
## EVAL Backlog（自动开单 per execution-loop §7.3 Rule 11）
...
<!-- codex-route:edge-flow-post-merge-git-issue -->
> Codex route: invoke `$mj-agent-git-issue` (native carrier; call, conditional)

## Reference Files

- [[../../../sdd/workflows/execution-loop|execution-loop]] §7（Post-merge sedimentation policy；§7.3 Rule 11 EVAL backlog mj-agent 专属）
- [[../../../sdd/lifecycle|lifecycle]] §2（Working-doc 生命周期，Step 9 plan state 改 completed 依据；§2.2 Stage 17 自动化 Rule 12）
- [[../../../sdd/lifecycle|lifecycle]] §2.1（Working 文档 frontmatter 4 态机段）
